...
export class ResourceUtil {
    ...
    public static getEffectPrefab (effectName: string, cb: Function) {
        this.loadRes(`prefab/effect/${effectName}`, Prefab, cb);
    }
     ...
     * @param cb 
     */
    public static getFighterModel (type: number, cb: Function) {
        let models: any = Constants.FIGHTER_MODEL;
        let name = models[type];
        if (type === Constants.FIGHTER_TYPE.CLOSE) {
            //基础兵-奥特曼，游戏包已包含，不需要下载子包
            this.loadRes(`prefab/model/man/${name}`, Prefab, cb);
        } else {
            //先做成统一子包下载，后续考虑优化成每个角色一个子包
            this.loadRes(`prefab/model/man/${name}`, Prefab, cb);
        }
    }
     ...
     * @param cb 
     */
    public static getWeapon (weaponName: string, cb: Function) {
        this.loadRes(`prefab/model/weapon/${weaponName}`, Prefab, cb);
    }

    /**
     * 获取士兵对应卡牌
     */
    public static getCard (type: number, cb: Function) {
        this.loadRes(`texture/card/${type}/spriteFrame`, SpriteFrame, cb);
    }
}
